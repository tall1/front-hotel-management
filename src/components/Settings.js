    ...
    const userId = await res.json();
     if  (checkValidity(data)){
      ...
      let interval = setInterval(async () => {
          const resStatus = await fetch(`http://localhost:8080/assignments/get_status/`+ 16) // should be stored task
          .then(function(body)
                {return(body.text());
                });
//          const jsonResStaus =await resStatus.;
        //  const a = JSON.parse(jsonResStaus)
         // console.log(a);
          if(resStatus === "DONE")
          {
            const assignRes = await fetch(`http://localhost:8080/assignments/get_assignment/`+ 16); // should be stored task
            const assignResJson =await assignRes.json();
            sessionStorage.setItem("assign", JSON.stringify(assignResJson));
            alert("your assignment is ready");
